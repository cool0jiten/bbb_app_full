...
class CallManager {
  ...
  String _buildUser() {
    ...
    return "${info.internalUserID}_$_audioSessionNumber-sarvID-${Uri.encodeComponent(info.fullUserName)}@${_getNakedUrl()}";
  }

  String _buildDisplayName() {
    // return "${info.internalUserID}_$_audioSessionNumber-bbbID-${info.fullUserName}";
    return "${info.internalUserID}_$_audioSessionNumber-sarvID-${info.fullUserName}";
  }
  ...
  String buildEcho() {
    ...
    return "${info.voiceBridge}@${_getNakedUrl()}";
  }
  ...
  UaSettings buildSettings({String transportScheme}) {
    ...
    settings.webSocketSettings.userAgent = 'SarvWave';
